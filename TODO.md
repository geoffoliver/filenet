...
- [x] Public key HTTP endpoint (`GET /pubkey`) — unauthenticated (removed, see below)
- [x] Remove `GET /pubkey` and `GET /health` — both unauthenticated on the P2P port with zero in-app consumers (confirmed by code search); the WebSocket `hello`/`hello-ack` handshake already delivers and cryptographically proves each peer's public key, so `/pubkey` was never load-bearing. Both endpoints let anyone mass-scan the default port and get a stable `nodeId` for free with a bare `curl`. Removing them collapses the pre-handshake surface to nothing — a scanner still sees the port is open, but can no longer fingerprint a node without actually implementing the WS handshake protocol.
...
- [x] Match Github pages branding (colors, fonts) to app — `site/style.css` now uses the app's exact tokens (blue `#3b82f6` primary, neutral gray/white surfaces, Geist Sans/Mono) instead of its own orange/teal/cream/IBM-Plex-Mono look; wordmark changed to match the app's "File"+blue-"net" treatment. Also brought `site/docs.html` back in sync with `README.md` (it had drifted): fixed the standalone-executable bundle description (missing the `server/` folder), added the env-var, fuller auto-update, and Security sections that existed in the README but not the docs site.
