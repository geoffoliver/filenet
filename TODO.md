...
- [ ] Output the UI URL at startup, alongside the existing Node ID: / P2P port: / UI port: log lines (e.g. UI: http://localhost:<UI_PORT>).
- [ ] Derive the websocket host from window.location instead of hardcoding localhost, so remote access from another machine on the network doesn't break.
